	...
	{	//step 2: insert the new node at the beginning
	 ...
	 else
	 {
		 ...
		 {							 //step 1.c traverse the list to find the correct position 
			 previous = current;	 //step  1.d: move the previouse to the current node 
		 ...
		 newNode->prev = previous; // step 5: make the previous field of the new node to previous

		 if (current != NULL)
		 {
			 current->prev = newNode; // step 6 make the previous field of the current node point the new node
		 }
		 if (previous != NULL)
		 {
...
void deleteNode()
{
	...
	//realese the memory of the node marked as current 
	delete current;
...
void searchData()
{
	...
	Node* prev, * curr;
	prev = curr = NULL;
	cout << "\\nEnter the number of the student whose record you want to search: "; int num;
	cin >> num;
...
int main()
{
	while (true)
	{
		try
		{
			...
			cout << "4.View all records in the ascending order of roll number" << endl;
			cout << "5. Search for a record in the list" << endl;
			cout << "6. Exit" << endl;
			cout << "nEnter ypur choice (1-6): ";
			char ch;
			cin >> ch;
		...
		{
			cout << "Check for the values enterd." << endl;
		}
	}
}